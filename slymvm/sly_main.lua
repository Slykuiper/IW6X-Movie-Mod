...
function callfunction(player)
	...
	elseif getdvarargs[1] == "icon" then
		...
		testhud:setshader(getdvarargs[2], 15, 15)
	...
	elseif getdvarargs[1] == "actortest" then
		if actortest ~= nil then
			actortest:delete()
		end
		actortest = game:spawn("script_model", player.origin)
		actortest.origin = player.origin
		actortest.angles = player.angles
		actortest:setmodel(player:getcustomizationbody())
		actortest:scriptmodelplayanim("mp_stand_idle")
	end
end